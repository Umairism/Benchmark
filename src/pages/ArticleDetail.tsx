...
const ArticleDetail: React.FC = () => {
  ...
  const fetchArticle = useCallback(async () => {
    ...
    try {
      ...
      if (article) {
        // Allow viewing if article is published OR if current user is the author
        if (article.published || (user && article.author_id === user.id)) {
          setArticle(article);
        } else {
          console.log('Article not published and user is not the author'); // Debug log
          setArticle(null);
        }
      } else {
        console.log('Article not found in database'); // Debug log
        setArticle(null);
      }
    } catch (error) {
